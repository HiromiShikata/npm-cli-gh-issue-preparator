...
6. If all jobs are finished and all CI jobs passed and all PR review comments are resolved, comment to issue "ALL_CI_PASSED, ALL_REVIEW_COMMENT_RESOLVED" and change status to ${params.awaitingQualityCheckStatus}
...
8. If all jobs are finished and all CI passed, and not all PR review comments are resolved, comment to issue "REVIEW_COMMENT_NOT_RESOLVED" and change status to ${params.preparationStatus}
