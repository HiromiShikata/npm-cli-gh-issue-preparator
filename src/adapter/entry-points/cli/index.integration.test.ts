...
describe('index', () => {
  ...
  describe('notifyFinishedIssuePreparation', () => {
    ...
    it('success', async () => {
      ...
      const result = execSync(
        'npx ts-node ./src/adapter/entry-points/cli/index.ts notifyFinishedIssuePreparation --projectUrl https://github.com/users/HiromiShikata/projects/49 --issueUrl https://github.com/HiromiShikata/test-repository/issues/1557 --preparationStatus "Preparation" --awaitingAutoQualityCheckStatus "Done" --awaitingQualityCheckStatus "Awaiting quality check" --commentCountThreshold 5',
        { encoding: 'utf-8', timeout: 600000 },
      );
      expect(result).toBeDefined();
      ...
      expect(afterIssue?.status).toBe('Done');
    }, 600000);
  });
});
