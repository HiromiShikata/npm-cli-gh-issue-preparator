...
export class GitHubIssueRepository implements IssueRepository {
  ...
  async findRelatedOpenPR(issueUrl: string): Promise<RelatedPullRequest[]> {
    ...
    while (hasNextPage) {
      ...
      for (const item of timelineItems) {
        ...
        const defaultBranchLatestCommit =
          pr.repository?.defaultBranchRef?.target?.history?.nodes[0]?.oid;
        const headRefOid = pr.headRefOid;

        let isBranchNotLatest = false;
        if (defaultBranchLatestCommit && headRefOid) {
          isBranchNotLatest = defaultBranchLatestCommit !== headRefOid;
        }
