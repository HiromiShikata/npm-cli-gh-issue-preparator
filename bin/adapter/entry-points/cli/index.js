...
program
    ...
    if (!configFileValues.projectUrl) {
        console.error('projectUrl must be set in the config YAML file when using startDaemon. The wrapper script reads project-scoped values directly from the config file. Providing it via --projectUrl CLI flag alone is not sufficient.');
        process.exit(1);
    }
    if (options.projectUrl &&
        options.projectUrl !== configFileValues.projectUrl) {
        console.error(`--projectUrl '${options.projectUrl}' does not match the projectUrl in the config file '${configFileValues.projectUrl}'. The wrapper reads projectUrl from the config file; a differing --projectUrl value would cause startDaemon to prepare issues in one project while the wrapper notifies status in another.`);
        process.exit(1);
    }
