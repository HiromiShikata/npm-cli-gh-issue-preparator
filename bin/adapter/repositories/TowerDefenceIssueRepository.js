...
class TowerDefenceIssueRepository {
    ...
    async getAllOpened(project) {
        ...
        return issues
            ...
            .map((issue) => this.mapToIssue(issue, project));
    }
    async get(issueUrl, project) {
        const { issues } = await this.loadData();
        const issue = issues.find((i) => i.url === issueUrl);
        if (!issue) {
            return null;
        }
        return this.mapToIssue(issue, project);
    }
    mapToIssue(towerDefenceIssue, _project) {
        return {
            nameWithOwner: towerDefenceIssue.nameWithOwner,
            number: towerDefenceIssue.number,
            title: towerDefenceIssue.title,
            state: towerDefenceIssue.state,
            status: towerDefenceIssue.status,
            story: towerDefenceIssue.story,
            nextActionDate: towerDefenceIssue.nextActionDate,
            nextActionHour: towerDefenceIssue.nextActionHour,
            estimationMinutes: towerDefenceIssue.estimationMinutes,
            dependedIssueUrls: towerDefenceIssue.dependedIssueUrls,
            completionDate50PercentConfidence: towerDefenceIssue.completionDate50PercentConfidence,
            url: towerDefenceIssue.url,
            assignees: towerDefenceIssue.assignees,
            labels: towerDefenceIssue.labels,
            org: towerDefenceIssue.org,
            repo: towerDefenceIssue.repo,
            body: towerDefenceIssue.body,
            itemId: towerDefenceIssue.itemId,
            isPr: towerDefenceIssue.isPr,
            isInProgress: towerDefenceIssue.isInProgress,
            isClosed: towerDefenceIssue.isClosed,
            createdAt: towerDefenceIssue.createdAt,
        };
    }
